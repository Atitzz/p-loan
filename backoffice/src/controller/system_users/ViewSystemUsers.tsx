...
function ViewSystemUsers() {
  ...
  const fetchData = (path) =>{
    setIsLoading(true);
    Get(`system/users/${path}?search=${search}&page=${page}&limit=${pageSize}`)
      .then((response) => {
  ...
  const viewDetails = (id) =>{
    const __route = dataRoute.find(x=>x.component == 'system_users_action')
  ...
  return (
    <Stack spacing={1}>
      <GridRender
        ...
        insertEnd={
          <Button variant="contained" startIcon={<Icons.Add/>} onClick={createUser}>{useLang("เพิ่มใหม่","Add New")}</Button>
        }
      />
    </Stack>
  );
