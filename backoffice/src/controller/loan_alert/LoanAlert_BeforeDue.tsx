...
function LoanAlert_BeforeDue() {
    ...
    const [debouncedSearchDay, setDebouncedSearchDay] = useState("");
    const timeoutRef = useRef(null);

    useEffect(() => {
        fetchData();
    }, [debouncedSearchDay, page, pageSize]);

    const fetchData = () => {
        setIsLoading(true);
        Get(`loan/alert/beforedue?searchDay=${debouncedSearchDay}&page=${page}&limit=${pageSize}`)
            .then((response) => {
    ...
    const columns: GridColDef<(typeof data)[number]>[] = [
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Chip
                    label={useLang("ลูกหนี้คงเหลือ", "Remaining Debt")}
                    variant="outlined"
                    color="secondary"
                    size="small"
                />
    ...
    const handleNotificationClick = () => {
        ...
        // กรองสินเชื่อที่ยังไม่ถูกแจ้งเตือนในวันนี้เท่านั้น
        const today = new Date();
        today.setHours(0, 0, 0, 0);
    ...
    return (
        <Stack spacing={1}>
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="md" fullWidth>
                <DialogTitle>
                    <Box sx={{ mb: 2 }}>{useLang("เลือกรายการที่จะส่ง", "Select for Notification")}</Box>
                    <TextField
                        fullWidth
                        value={searchText}
...
export default LoanAlert_BeforeDue;