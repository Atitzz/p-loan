...
function ToolsProvider({ children }: { children: React.ReactNode }) {
  ...
  const __export = {
    ...
    calcurateAge: (birthDate: string) => {
      ...
      // คำนวณส่วนที่เหลือของเดือนหลังจากคำนวณปีแล้ว
      const millisecondsPerMonth = 1000 * 60 * 60 * 24 * 30.44; // ใช้ค่าเฉลี่ย 30.44 วันต่อเดือน
      const remainingMilliseconds =
        differenceInMilliseconds - years * millisecondsPerYear;
    ...
    paymentCalculator: (
      ...
      const __installment_per_month = Math.round((__diff2 / __pill2 + Number.EPSILON) * 100) / 100;
      let nextDate = new Date(installment_date || new Date().toISOString().split('T')[0]);
      let currentDate = new Date(new Date(payment_date).toISOString().split('T')[0] || new Date().toISOString().split('T')[0]);
