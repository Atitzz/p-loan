...
function GridRender({
  ...
  }, [paginationModel,__location.pathname]);

  const [columnVisibilityModel, setColumnVisibilityModel] =
  React.useState<GridColumnVisibilityModel>(columns.reduce((acc, column) =>{
    
    if(column.hide) acc[column.field] = false;
    else acc[column.field] = true;
    return acc
  },{}));
  return (
    <Stack gap={4} >
        ...
        >
          ...
          <Stack direction="row" spacing={2}>
             {onSearchText && <TextField
              size="small"
              placeholder={useLang("ค้นหา...","Search")}
              value={searchText}
              onChange={onSearch}
      ...
      {/* <Stack sx={{flex:1,maxWidth:'calc(100vw - 280px)',overflowX:'auto'}}> */}
      <DataGrid
