...
function LoanDue() {
  ...
  useEffect(() => {
    Get(`loan/installment/due/${loan_id}`).then((res) => {
  ...
  const onOpenDetails = (id) => {
    setDialogDetails(true);
    // ใช้ index แทนการใช้ id เพื่อให้ได้ข้อมูลจาก array เดียวกับที่แสดงในหน้า LoanDue
    const installmentIndex = data.findIndex((_, index) => index === id);
    if (installmentIndex === -1) {
      AddAlert("ไม่พบข้อมูลที่คุณเลือก", "error");
      return;
    }
    setDetails(data[installmentIndex]);
  };
                ...
                >
                  ...
                  >
                    ...
                    <Stack direction="row" sx={{ alignItems: "center" }}>
                      <Typography variant="h6" children={toTHB(row.amount)} />
                      <Box sx={{ p: 2 }}>
                        {/* <IconButton onClick={() => onOpenDetails(row.id)}> */}
                        <IconButton onClick={() => onOpenDetails(index)}>
                          <ArrowForwardIos />
                        </IconButton>
                      </Box>
                    </Stack>
